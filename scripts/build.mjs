...
await cp(join(root, "src", "executive-positioning.css"), join(out, "assets", "executive-positioning.css"));
...
<head>
  ...
  <link rel="stylesheet" href="/assets/executive-positioning.css">
...
function footer() {
  return `<footer class="site-footer"><div class="footer-inner"><div class="footer-top"><div class="footer-title">Muhammad Haris Aslam<br>GCC operator and business builder.</div><nav class="footer-links" aria-label="Due diligence"><p>Due diligence</p><a href="/operating-record" data-cta-id="operating_record_view">Operating Record</a><a href="/family-business" data-cta-id="family_business_view">For Business Owners</a><a href="/executive-profile" data-cta-id="executive_profile_view">Executive Profile</a><a href="/executive-search" data-cta-id="executive_search_view">Executive Search</a></nav><nav class="footer-links" aria-label="Work and contact"><p>Work and contact</p><a href="/use-cases">Use Cases</a><a href="/ai-transformation">AI & Transformation</a><a href="/operator-notes">Operator Notes</a><a href="/about">About</a><a href="/contact" data-cta-id="contact_click">Contact</a><a href="${site.linkedin}" rel="me noopener" data-cta-id="linkedin_click">LinkedIn</a></nav></div><div class="footer-meta"><span>© ${new Date().getUTCFullYear()} ${site.name}</span><span>GCC operating experience</span></div></div></footer>`;
}
function render404() {
...
function breadcrumbItems(path) {
  ...
  if (path.startsWith("/track-record/")) {
    ...
    return [["Home", "/"], ["Operating Record", "/operating-record"], [page?.h1 || "Operating story", path]];
  }
  ...
  const hubLabels = { "/track-record":"Track Record", "/operating-record":"Operating Record", "/family-business":"For Business Owners", "/executive-profile":"Executive Profile", "/operator-notes":"Operator Notes", "/use-cases":"Use Cases", "/ai-transformation":"AI & Transformation", "/insights":"Insights" };
...
function schemaGraph(page, url, crumbs) {
  const person = { "@type":"Person", "@id":`${site.origin}/#person`, name:site.name, url:`${site.origin}/executive-profile`, image:`${site.origin}/assets/haris-aslam.webp`, sameAs:[site.linkedin], jobTitle:"GCC operator and business builder", description:"GCC operator and business builder focused on turnaround, growth, digital commerce, commercial transformation and venture building.", knowsAbout:["Retail turnaround","Commercial transformation","Marketplace operating models","Digital commerce","GCC growth","Family-business transformation","Venture building","Applied AI","Payments and logistics"], homeLocation:{"@type":"Place",name:"Doha, Qatar"} };
...
function current(path, href) { return path === href || (href !== "/" && path.startsWith(`${href}/`)) || (href === "/operating-record" && path.startsWith("/track-record/")) || (href === "/use-cases" && ["/ai-commerce","/career-runway"].includes(path)); }
