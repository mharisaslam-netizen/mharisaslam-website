...
<main>
  ...
  <div class="grid">
    <form class="card" method="post" action="/api/linkedin/publish" id="publisherForm">
      ...
      <div class="field"><label for="articleUrl">Article URL</label><input type="text" id="articleUrl" name="articleUrl" value="${esc(first.url)}" required></div>
...
function render(){count.textContent=text.value.length;preview.textContent=text.value;previewTitle.textContent=title.value;previewDescription.textContent=description.value;previewUrl.textContent=url.value}
select.addEventListener("change",()=>{const d=drafts.find(x=>x.id===select.value);if(!d)return;text.value=d.text;url.value=d.url;title.value=d.title;description.value=d.description;approve.checked=false;button.disabled=true;render()});
for(const el of [text,url,title,description]) el.addEventListener("input",()=>{approve.checked=false;button.disabled=true;render()});
